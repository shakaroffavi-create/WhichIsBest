(()=>{
  ...
  function loadExcelLibrary(){
    ...
    return new Promise((resolve,reject)=>{const existing=document.querySelector('script[data-exceljs]');if(existing){existing.addEventListener('load',()=>resolve(globalThis.ExcelJS),{once:true});existing.addEventListener('error',reject,{once:true});return}const script=document.createElement('script');script.dataset.exceljs='1';script.src='https://cdn.jsdelivr.net/npm/exceljs@4.4.0/dist/exceljs.min.js';script.crossOrigin='anonymous';script.onload=()=>globalThis.ExcelJS?resolve(globalThis.ExcelJS):reject(Error('ספריית Excel לא נטענה'));script.onerror=()=>reject(Error('ספריית Excel לא נטענה'));document.head.append(script)})
  }
  async function exportExcel(){
    ...
    const button=$('exportExcel');if(button){button.disabled=true;button.textContent='מכין קובץ Excel…'}
    try{
      const ExcelJS=await loadExcelLibrary(),workbook=new ExcelJS.Workbook();workbook.creator='WICHISBEST.COM';workbook.created=new Date();
      const sheet=workbook.addWorksheet('מטריצת השוואה',{views:[{rightToLeft:true,state:'frozen',ySplit:3,showGridLines:false}],pageSetup:{orientation:'landscape',fitToPage:true,fitToWidth:1,fitToHeight:0}}),columnCount=Math.max(...rows.map(row=>row.length),1);
      sheet.mergeCells(1,1,1,columnCount);sheet.getCell(1,1).value='WICHISBEST.COM — מטריצת השוואת הצעות';sheet.mergeCells(2,1,2,columnCount);sheet.getCell(2,1).value=`הופק בתאריך ${new Intl.DateTimeFormat('he-IL').format(new Date())}`;
      rows.forEach((row,index)=>sheet.addRow(row));sheet.columns.forEach((column,index)=>{column.width=index===0?28:22});sheet.getRow(1).height=32;sheet.getRow(2).height=22;sheet.getRow(3).height=28;
      sheet.getCell(1,1).style={font:{name:'Arial',size:17,bold:true,color:{argb:'FFFFFFFF'}},fill:{type:'pattern',pattern:'solid',fgColor:{argb:'FF092F49'}},alignment:{horizontal:'center',vertical:'middle',readingOrder:'rtl'}};
      sheet.getCell(2,1).style={font:{name:'Arial',size:10,color:{argb:'FF365867'}},fill:{type:'pattern',pattern:'solid',fgColor:{argb:'FFEAF6F8'}},alignment:{horizontal:'center',vertical:'middle',readingOrder:'rtl'}};
      const border={top:{style:'thin',color:{argb:'FFD4E4E9'}},left:{style:'thin',color:{argb:'FFD4E4E9'}},bottom:{style:'thin',color:{argb:'FFD4E4E9'}},right:{style:'thin',color:{argb:'FFD4E4E9'}}};
      for(let row=3;row<=sheet.rowCount;row++)for(let col=1;col<=columnCount;col++){const cell=sheet.getCell(row,col);cell.font={name:'Arial',size:11,bold:row===3||col===1,color:{argb:row===3?'FFFFFFFF':col===1?'FF092F49':'FF102F42'}};cell.fill={type:'pattern',pattern:'solid',fgColor:{argb:row===3?'FF0B5C74':col===1?'FFEAF6F8':row%2===0?'FFF7FBFC':'FFFFFFFF'}};cell.alignment={horizontal:col===1?'right':'center',vertical:'middle',wrapText:true,readingOrder:'rtl'};cell.border=border}
      sheet.autoFilter={from:{row:3,column:1},to:{row:3,column:columnCount}};const buffer=await workbook.xlsx.writeBuffer(),blob=new Blob([buffer],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`WICHISBEST-${new Date().toISOString().slice(0,10)}.xlsx`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);localStorage.setItem('wib_meaningful_use','1');localStorage.setItem('wib_export_count',String(Number(localStorage.getItem('wib_export_count')||0)+1));setTimeout(maybeFeedback,700)
    }catch(error){console.error(error);alert('לא הצלחנו ליצור את קובץ Excel. בדקו את החיבור לאינטרנט ונסו שוב.')}finally{if(button){button.disabled=false;button.textContent='ייצוא דוח מעוצב ל־Excel'}}
  }
  function installExport(){const actions=document.querySelector('.result-actions');if(!actions||$('exportExcel'))return;const button=document.createElement('button');button.id='exportExcel';button.type='button';button.className='secondary-action export-excel';button.textContent='ייצוא דוח מעוצב ל־Excel';button.onclick=exportExcel;actions.insertBefore(button,actions.lastElementChild)}
