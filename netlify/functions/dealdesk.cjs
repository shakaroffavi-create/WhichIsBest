...
const {detectProcurementProfile,buildProfessionalRisks,buildProfessionalActions}=require('./_procurement-knowledge.cjs');
...
exports.handler=async event=>{
  ...
  try{
    ...
    const professionalProfile=detectProcurementProfile(p);
    ...
    risks.push(...buildProfessionalRisks(p,professionalProfile));
    const actions=[
    ...
    actions.push(...buildProfessionalActions(p,professionalProfile));
    ...
    return reply(200,{bottomLine,confidence,risks:risks.slice(0,6),actions:actions.slice(0,5),professionalProfile:{id:professionalProfile.id,name:professionalProfile.name,isHvac:professionalProfile.isHvac}});
